      ...
      <nav className="glass px-6 py-4 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="p-2.5 rounded-xl hover:bg-dark-800 transition-colors"
            title="Logout"
          >
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-10">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3 px-4 py-3 bg-dark-800/50 rounded-xl border border-dark-700/50">
                  ...
                  <button
                    ...
                    className={`w-10 h-5 rounded-full relative transition-colors ${isPrivate ? "bg-primary-500" : "bg-dark-600"}`}
                  >
                    <div
                      className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-transform ${isPrivate ? "right-0.5" : "left-0.5"}`}
                    ></div>
                  </button>
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={loading}
                    className="flex-1 py-2.5 bg-primary-600 hover:bg-primary-500 rounded-xl font-semibold transition-all disabled:opacity-50"
                  >
                    {loading ? "..." : "Start"}
                  </button>
                  <button
                    onClick={() => setShowCreate(false)}
                    className="px-4 py-2.5 glass-light rounded-xl font-medium"
                  >
                    X
                  </button>
                </div>
          ...
          >
            ...
            >
              <Calendar className="w-5 h-5 text-accent-400" /> Schedule for
              later
            </button>
          </div>
          ...
          >
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Code"
                className="flex-1 px-4 py-3 bg-dark-800 border border-dark-700 rounded-xl focus:outline-none focus:border-primary-500 uppercase font-mono text-center"
                maxLength={8}
              />
              <button
                onClick={handleJoin}
                className="px-5 py-3 bg-dark-800 hover:bg-dark-700 border border-dark-700 rounded-xl transition-all"
              >
                Join
              </button>
            </div>
          ...
          <div className="mb-10 animate-slide-up">
            ...
            <div className="grid gap-4">
              {scheduledMeetings.map((m) => (
                <div
                  key={m._id}
                  className="glass rounded-xl p-5 hover:border-accent-500/30 transition-all group border-l-4 border-l-accent-500"
                >
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-accent-500/10 flex flex-col items-center justify-center text-accent-400">
                        <span className="text-xs font-bold uppercase">
                          {new Date(m.scheduledAt).toLocaleString("default", {
                            month: "short",
                          })}
                        </span>
          ...
          {history.length === 0 ? (
            ...
            <div className="grid gap-3">
              {history.map((m) => (
                <div
                  key={m._id}
                  className="glass rounded-xl px-5 py-4 hover:bg-white/5 transition-all flex items-center justify-between group border border-dark-800/50"
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-2 h-2 rounded-full ${m.isActive ? "bg-green-400 animate-pulse" : "bg-dark-700"}`}
                    ></div>
                    <div>
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {m.isActive && (
                      <button
                        ...
                        className="px-4 py-1.5 bg-primary-600/20 hover:bg-primary-600/30 text-primary-400 rounded-lg text-xs font-bold border border-primary-600/50"
                      >
                        Rejoin
                      </button>
                    )}
                    {!m.isActive && m.host?._id === user?._id && (
                      <button
                        onClick={() => openAttendance(m.roomId)}
                        className="px-4 py-1.5 bg-yellow-500/20 hover:bg-yellow-500/30 text-yellow-300 rounded-lg text-xs font-bold border border-yellow-500/40"
                      >
                        Attendance
                      </button>
                    )}
      ...
      {attendanceRoomId && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div
            ...
            onClick={() => setAttendanceRoomId(null)}
          ></div>
          <div className="glass rounded-2xl w-full max-w-3xl h-[80vh] relative animate-scale-in overflow-hidden">
            <div className="px-6 py-4 border-b border-dark-800 flex items-center justify-between">
              <h3 className="text-lg font-bold">Attendance Result</h3>
              <button
                onClick={() => setAttendanceRoomId(null)}
          ...
          <div className="glass rounded-2xl w-full max-w-md p-8 relative animate-scale-in">
            ...
            <form onSubmit={handleSchedule} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 bg-gradient-to-r from-primary-600 to-accent-600 hover:from-primary-500 hover:to-accent-500 rounded-xl font-semibold transition-all duration-300 disabled:opacity-50"
              >
