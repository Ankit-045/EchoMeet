...
export default function SummaryPanel({ roomId, onGenerate, transcript }) {
  ...
  useEffect(() => { fetchSummaries(); }, [roomId]);
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {loading ? (
          ...
          <div className="text-center text-dark-500 text-sm py-8">
            ...
            <p className="text-xs mt-1">Start recording, then generate a summary</p>
          </div>
        ) : (
          summaries.map((s, i) => (
            <div key={s._id || i} className="glass-light rounded-xl p-4 space-y-3 animate-fade-in">
              <div className="flex items-center justify-between">
                <span className="text-xs text-dark-500">
                  {new Date(s.generatedAt).toLocaleString()}
                </span>
                {s.participantCount && (
                  <span className="text-xs text-dark-500">{s.participantCount} participants</span>
                )}
              </div>
              ...
              {s.summary && (
                <div>
                  ...
                  <p className="text-sm text-dark-200 leading-relaxed">{s.summary}</p>
                </div>
              )}
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    {s.actionItems.map((ai, j) => (
                      <li key={j} className="text-sm text-dark-300 p-2 rounded-lg bg-dark-800/50">
                        <p>{ai.item}</p>
